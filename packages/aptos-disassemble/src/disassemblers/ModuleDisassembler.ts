...
export class ModuleDisassembler {
    ...
    private buildModuleAliases(): { moduleNames: Map<string, number>; moduleAliases: Map<string, string> } {
    ...
    private generateImports(moduleNames: Map<string, number>, moduleAliases: Map<string, string>): string {
        const rawModule = this.context.getRawModule();
        const selfModuleAddress = this.context.getSelfModuleAddress();
        const selfModuleName = this.context.getSelfModuleName();
        ...
        return rawModule.function_defs.map((functionDefinition) => {
            ...
            if (functionDefinition.code === undefined) {
                ...
                functionDefinition.code.code.forEach((instruction, idx) => {
                    ...
                    body.push(`${"".padStart(4, " ")}${`${idx}`.padEnd(4, " ")}:${"".padEnd(6, " ")}${instructionStr}`);
                });
            }
